	...
	if [[ $varsys == *"fedora"* ]]; then
		...
		if [[ $varsys == *"Debian"* || $varsys == *"debian"* ]]; then
			...
			if [[ $var0 == *"Raspberry"* || $var0 == *"raspberry"* ]]; then
				...
				if [[ "$var55" =~ "fastfetch 2.15.0 (x86_64)" ]]; then
					echo ""
					echo "Fastfetch est à jour"
				else
					echo ""
					echo "cd || return && wget -O - https://github.com/fastfetch-cli/fastfetch/releases/download/2.15.0/fastfetch-linux-amd64.deb > fastfetch-linux-amd64.deb && sudo dpkg -i fastfetch-linux-amd64.deb ; sudo rm -rv fastfetch-linux-amd64.deb"
					cd || return && wget -O - https://github.com/fastfetch-cli/fastfetch/releases/download/2.15.0/fastfetch-linux-amd64.deb > fastfetch-linux-amd64.deb && sudo dpkg -i fastfetch-linux-amd64.deb ; sudo rm -rv fastfetch-linux-amd64.deb
				fi
			fi		
		fi
	fi
